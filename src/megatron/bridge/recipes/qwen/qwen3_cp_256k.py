...
---------------------------------
Short validation run to prove CP>1 works at 256k on Qwen3-8B (dense transformer,
...
Parallelism: TP=4, CP=4, PP=1  ->  16 GPUs/replica, 4 DP replicas (16 nodes)
             Change CONTEXT_PARALLEL_SIZE=8 for the next validation pass.
"""
...
CONTEXT_PARALLEL_SIZE = 4    # 4 for 256k pass
...
def qwen3_8b_cp_validation_config(hf_model_path: str = "/mnt/vast/proj/checkpoints/bathen/models/base/Qwen3-8B-Base") -> ConfigContainer:
    ...
    After validating CP=4, change CONTEXT_PARALLEL_SIZE=8 and re-run.
    """
    cfg = _sft_common()
    ...
    # -- packed_sequence_specs: pad_seq_to_mult = CP * 2 ---------------------
