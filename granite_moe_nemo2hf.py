#!/usr/bin/env python3
"""Full Granite MoE SFT export: Megatron → HuggingFace (transmuted).

Clean single-process or multi-process export — no subprocesses, no embedded scripts.
Calls Bridge APIs directly under torchrun.

Pipeline:
1. Load Megatron model via Bridge (all ranks, TP/EP-aware)
2. Bridge export to HF format (QKV de-interleaving, multiplier un-baking, expert stacking)
3. Patch lm_head if Bridge dropped it (tied embeddings workaround)
4. Transmute (re-bake multipliers into weights, config=1.0, sharded safetensors)
5. Verify (NaN/Inf, config, lm_head)
...
- Expert weights as stacked [num_experts, ...] tensors in HF format
- Sharded safetensors for large models

Usage:
    # Single GPU test (reshards from TP/EP internally)
    torchrun --nproc-per-node=1 granite_moe_nemo2hf.py \
        --hf-config /path/to/granitemoe-base \
        --megatron-path /path/to/checkpoint/iter_XXXX \
        --output /path/to/hf/export

    # Multi-GPU matching training parallelism (TP=2, EP=2)
    torchrun --nproc-per-node=4 granite_moe_nemo2hf.py \
        --hf-config /path/to/granitemoe-base \
        --megatron-path /path/to/checkpoint/iter_XXXX \
        --output /path/to/hf/export \
        --tp 2 --ep 2
...
def patch_lm_head(bridge_output: str, model, tp_size: int, hf_config: str):
    ...
    Bridge drops lm_head when tie_word_embeddings=True in the original HF
    config. This extracts output_layer from the Megatron model (already in
    memory), un-bakes logits_scaling, and patches it in.
    """
...
def transmute(bridge_output: str, final_output: str, max_shard_size: str = "5GB"):
    ...
    MoE variant: handles block_sparse_moe.output_linear.weight (replaces
    dense mlp.down_proj) and keeps o_proj handling (shared between dense/MoE
    for the attention path).
    """
    print(f"\n{'='*60}")
    print("Transmute (bake multipliers -> config=1.0) [MoE]")
    print(f"{'='*60}")
    ...
    print(f"  Model type: GraniteMoE ({num_experts} experts)")
    print(f"  Baking: embed*={m_e}, o_proj*={m_r}, output_linear*={m_r}, lm_head/={m_l}")
        ...
        with safe_open(str(sf_path), framework="pt", device="cpu") as sf:
            for key in sf.keys():
                ...
                if key == "model.embed_tokens.weight" and m_e != 1.0:
                    ...
                    t = (t.float() * m_r).to(t.dtype)
                    bake_count += 1
                elif re.search(r"block_sparse_moe\.output_linear\.weight$", key) and m_r != 1.0:
                    t = (t.float() * m_r).to(t.dtype)
                    bake_count += 1
    ...
    for f_path in src.iterdir():
        if f_path.name != "config.json" and not f_path.name.endswith(".safetensors"):
            shutil.copy2(f_path, dst / f_path.name)
...
def verify(final_output: str):
    """Sanity check on the exported MoE model."""
    print(f"\n{'='*60}")
    print("Verify")
    print(f"{'='*60}")
    ...
    num_experts = config.get("num_local_experts", 0)
    checks.append((f"num_local_experts={num_experts}", num_experts > 0))
    ...
    for sf_path in sorted(dst.glob("*.safetensors")):
        with safe_open(str(sf_path), framework="pt", device="cpu") as sf:
            ...
            for key in sf.keys():
                if "block_sparse_moe.output_linear.weight" in key:
                    expert_weight_count += 1

    checks.append(("lm_head.weight exists", has_lm_head))
    checks.append(("embed_tokens.weight exists", has_embed))
    checks.append((f"MoE output_linear weights found ({expert_weight_count})", expert_weight_count > 0))
    ...
    if all_pass:
        print(f"\n  All checks passed! MoE export ready: {final_output}")
    else:
        print(f"\n  SOME CHECKS FAILED")
...
def main(
    ...
    print(f"\n{'#'*60}")
    print(f"# GRANITE MOE FULL EXPORT")
    ...
    print(f"\n{'='*60}")
    print("Phase 1: Bridge export (QKV + multiplier un-baking + expert stacking)")
    print(f"{'='*60}")
    ...
    mp.expert_model_parallel_size = ep_size
    ...
    if rank == 0:
        ...
        print(f"\n{'#'*60}")
        if passed:
            print(f"# MOE EXPORT SUCCESSFUL: {output}")
            print(f"#")
            print(f"# Serve with:")
            ...
            print(f"# MOE EXPORT COMPLETED WITH WARNINGS")
        print(f"{'#'*60}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Full Granite MoE SFT export: Megatron -> HuggingFace (transmuted)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Single GPU (reshards internally)
    torchrun --nproc-per-node=1 granite_moe_nemo2hf.py \\
        --hf-config /path/to/granitemoe-base \\
        --megatron-path /path/to/checkpoint/iter_XXXX \\
        --output /path/to/hf/export

    # Multi-GPU with TP=2, EP=2
    torchrun --nproc-per-node=4 granite_moe_nemo2hf.py \\
        --hf-config /path/to/granitemoe-base \\
        --megatron-path /path/to/checkpoint/iter_XXXX \\
        --output /path/to/hf/export \\
    ...
    parser.add_argument("--hf-config", required=True,
                        help="Path to base HF GraniteMoe model (for config + tokenizer)")
    ...
    parser.add_argument("--tp", type=int, default=0,
                        help="Tensor parallel size (default: auto from torchrun world size / ep)")
